...
static time_t sStopTimeInSec = 0;
static uint16_t sStopTimeInMilliSec = 0;
static time_t sOffsetTimeInSec = 0;
static uint16_t sOffsetTimeInMilliSec = 0;
...
static void calculateTimeSinceStart() {
  ...
  if (timeInMilliSec < sStartTimeInMilliSec) {
    sRecordTimingInMilliSec = timeInMilliSec + 1000 - sStartTimeInMilliSec + sOffsetTimeInMilliSec;
    timeInSec -= 1;
  }
  else {
  ...
  if (sOldTimeInSec != timeInSec) {
    sSecTimeChanged = true;
    sRecordTimingInSec = timeInSec - sStartTimeInSec + sOffsetTimeInSec;
...
static void calculateOffsetTime() {
  time_ms(&sStopTimeInSec, &sStopTimeInMilliSec);
  sOffsetTimeInSec += sStopTimeInSec - sStartTimeInSec;
  sOffsetTimeInMilliSec += sStopTimeInMilliSec - sStartTimeInMilliSec;
}
...
void configProvider(Window *window) {
  sRecordTimingInSec = 0;
...
static void init() {
  ...
  window_set_click_config_provider(sMainWindow, (ClickConfigProvider) configProvider);
}
